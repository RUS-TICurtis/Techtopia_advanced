...
export default function Analytics() {
  ...
  // Dynamic calculations based on mock database
  ...
  const totalWonValue = wonDeals.reduce((acc, curr) => acc + curr.value, 0);
  const avgDealSize = totalDealsCount > 0 ? (deals.reduce((acc, curr) => acc + curr.value, 0) / totalDealsCount) : 0;
  
  // Conversion win rate: won / (won + lost)
  const totalClosed = wonDeals.length + lostDeals.length;
  const conversionRate = totalClosed > 0 ? ((wonDeals.length / totalClosed) * 100) : 64.2;
  ...
  const funnelStages = ['Lead', 'Qualified', 'In Progress', 'Proposal', 'Won'];
  const stageColors = {
    'Lead': '#8A4FFF',      // Purple
    'Qualified': '#3772FF', // Blue
    'In Progress': '#E4FF1A', // Chartreuse
  ...
  const funnelData = funnelStages.map(stage => {
    ...
    return {
      ...
      value: stageDeals.reduce((acc, curr) => acc + curr.value, 0),
